...
def normalize_yahoo(df):
	...
	titles = ['symbol', 'company_name', 'Unnamed: 2','Price(extra)', 'change', 'perc_change', 'volume', '3_Month_Avg_Volume', 'Market Cap', 'P/E_Ratio (TTM)', '52_Wk_Change_%', '52_Wk_Range']
	df.columns = titles
	...
	df.drop(['Unnamed: 2', 'Price(extra)', '3_Month_Avg_Volume', 'Market Cap', 'P/E_Ratio (TTM)', '52_Wk_Change_%', '52_Wk_Range'], axis=1, inplace=True)
	...
	df[numeric_cols] = df[numeric_cols].replace(['nan', 'nanM'], None).fillna(np.nan) # convert to np.nan and infer type to handle deprecated behavior
...
def normalize_wsj(df):
	...
	df.rename(columns={'Unnamed: 0': 'company_name', 'Volume': 'volume', 'Last': 'price', 'Chg': 'change', '% Chg': 'perc_change'}, inplace=True)
